...
cells = [
 ...
 code("import glob, os, shutil, subprocess, sys, time",
      ...
      "        if not man:",
      "            continue",
      "        want = len(_pd.read_parquet(man[0]))",
      "        jpg = glob.glob(f'{r}/**/*.jpg', recursive=True)",
      "        png = glob.glob(f'{r}/**/*.png', recursive=True)",
      "        # Complete means matching the MANIFEST, not merely jpg==png. A",
      "        # truncated upload can be balanced (13,960 jpg AND 13,960 png)",
      "        # and would pass a jpg==png test while being 12% of the data.",
      "        if len(jpg) == want and len(png) == want:",
      "            return os.path.dirname(man[0]), 'kaggle dataset (pre-extracted)'",
      "        if jpg or png:",
      "            print(f'skipping {r}: {len(jpg)} jpg / {len(png)} png '",
      "                  f'vs {want} in manifest (incomplete)')",
      "",
      "    # 2. A .zip that Kaggle did NOT auto-expand (unlike .tar, Kaggle",
      "    #    leaves .zip archives intact -- which is exactly why the data",
      "    #    should be uploaded zipped: 1 file, not 232,140).",
      "    for r in roots:",
      "        z = glob.glob(f'{r}/**/*.zip', recursive=True)",
      "        if z:",
      "            return r, 'kaggle dataset (zip)'",
      "",
      "    # 3. Loose .tar archives.",
      "    for r in roots:",
 ...
 code("import os, glob, shutil, subprocess, time",
      ...
      "",
      "    # A .zip of the shards: unzip it first, then treat its contents as",
      "    # the source directory of .tar shards.",
